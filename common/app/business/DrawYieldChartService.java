 ...
 * Time: 上午11:12
 */
public class DrawYieldChartService {
    /**
     *
     ...
     */
    public static F.T2<Date,Date> getMinAndMaxDate(boolean isHistory,String strategyId,Date upTime){
        Date now = new Date();
        String queryHistoryDateSql = SqlLoader.getSqlById("queryHistoryUpdateDate");
        String queryRealTimeMinUpdateDateSql = SqlLoader.getSqlById("queryRealTimeMinUpdateDate");
        Map<String, Object> maxMinHistoryUpdateDateMap = QicDbUtil.queryQicDBSingleMap(queryHistoryDateSql, strategyId);
        Map<String, Object> minRealTimeMinUpdateDateMap = QicDbUtil.queryQicDBSingleMap(queryRealTimeMinUpdateDateSql, strategyId);
        Date maxDate;
        Date minDate;
        ...
        if(maxMinHistoryUpdateDateMap!=null ){
            minDateForHistory = maxMinHistoryUpdateDateMap.get("minDate")==null?now:(Date)maxMinHistoryUpdateDateMap.get("minDate");
            maxDateForHistory = maxMinHistoryUpdateDateMap.get("maxDate")==null?now:(Date)maxMinHistoryUpdateDateMap.get("maxDate");
        }
        if(minRealTimeMinUpdateDateMap!=null ){
            minDateForRealTime = upTime==null?now:upTime;
            maxDateForRealTime = minRealTimeMinUpdateDateMap.get("maxDate")==null?now:(Date)minRealTimeMinUpdateDateMap.get("maxDate");
        }
        if(minDateForRealTime.after(maxDateForRealTime)){
            minDateForRealTime = maxDateForRealTime;
        }
        if(isHistory){
    ...
    public static  String[] strategyDetailForDayPictrue(String strategyId,boolean isHistory,Date upTime){
        ...
        //取最近三个月的收益率
        List<StrategyDailyYieldDto>  yield_list_for_day = QicDbUtil.queryQicDBBeanList(pictrue_sql_for_day, StrategyDailyYieldDto.class, strategyId, minDate, maxDate);
    ...
    public static  String[] strategyDetailForWeekPictrue(String strategyId,boolean isHistory,Date upTime){
        String[] arr_strategys = new String[5] ;
        if (strategyId==null){
            return arr_strategys;
        }
        ...
        Date maxDate = getMinAndMaxDate(isHistory,strategyId,upTime)._1;//最大时间
        //取一段时间的收益率
        List<StrategyDailyYieldDto> yield_list_for_week = QicDbUtil.queryQicDBBeanList(pictrue_sql_for_week, StrategyDailyYieldDto.class, strategyId, minDate, maxDate);
