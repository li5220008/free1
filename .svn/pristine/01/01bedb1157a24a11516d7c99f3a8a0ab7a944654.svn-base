...
public class ActivateUserService {
    ...
    public static F.T2<List<ActivateUserDto>, Page> userList(AvtivatePar ap, int pageNo) {
        ...
        if(StringUtils.isNotBlank(ap.name)&&StringUtils.isNotBlank(ap.account)){
            total = QicDbUtil.queryQicDbCount(coutSql.toString(), "%" + ap.name + "%", "%" + ap.account + "%");
        }else if(StringUtils.isNotBlank(ap.name)&&StringUtils.isBlank(ap.account)){
            total = QicDbUtil.queryQicDbCount(coutSql.toString(), "%" + ap.name + "%");
        }else if(StringUtils.isBlank(ap.name)&&StringUtils.isNotBlank(ap.account)){
            total = QicDbUtil.queryQicDbCount(coutSql.toString(), "%" + ap.account + "%");
        }else{
        ...
        if (total > 0) {
            if(StringUtils.isNotBlank(ap.name)&&StringUtils.isNotBlank(ap.account)){
                audList = QicDbUtil.queryQicDBBeanList(sqlList, ActivateUserDto.class, "%" + ap.name + "%", "%" + ap.account + "%");
            }else if(StringUtils.isNotBlank(ap.name)&&StringUtils.isBlank(ap.account)){
                audList = QicDbUtil.queryQicDBBeanList(sqlList, ActivateUserDto.class, "%" + ap.name + "%");
            }else if(StringUtils.isBlank(ap.name)&&StringUtils.isNotBlank(ap.account)){
                audList = QicDbUtil.queryQicDBBeanList(sqlList, ActivateUserDto.class, "%" + ap.account + "%");
            }else{
                audList = QicDbUtil.queryQicDBBeanList(sqlList, ActivateUserDto.class);
            }
        }
